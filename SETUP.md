...
This file is for Claude Code (or any AI coding agent). When a user asks you to install or set up Goblin, follow these steps in order.
...
```bash
bash <path-to-goblin>/setup.sh
```

Replace `<path-to-goblin>` with the actual path where this repo was cloned. If you cloned it to a temp directory, use that path.
...
You can preview what will happen without making changes by running `bash <path-to-goblin>/setup.sh --dry-run` first.
