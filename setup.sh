...
#   git clone https://github.com/Voxos-ai-Inc/goblin.git && cd tas && bash setup.sh
#
# Flags:
#   --dry-run     Print what would be done without making changes
#   --uninstall   Remove Goblin from the current project
#
...
info()  { echo -e "${CYAN}[goblin]${NC} $1"; }
ok()    { echo -e "${GREEN}[goblin]${NC} $1"; }
warn()  { echo -e "${YELLOW}[goblin]${NC} $1"; }
fail()  { echo -e "${RED}[goblin]${NC} $1"; exit 1; }
...
if [ ! -d "$SCRIPT_DIR/hooks" ]; then
  info "Downloading Goblin files..."
  GOBLIN_TMPDIR=$(mktemp -d)
  git clone --depth 1 https://github.com/Voxos-ai-Inc/goblin.git "$GOBLIN_TMPDIR/tas" 2>/dev/null
  SCRIPT_DIR="$GOBLIN_TMPDIR/tas"
fi
...
if [ "$UNINSTALL" = true ]; then
  echo ""
  info "Uninstalling Goblin from $REPO_ROOT"
  echo ""
