...
def setup_workspace(task, condition: str, trial: int) -> Path:
    ...
    # Inject Goblin files for the Goblin condition
    if condition == GOBLIN:
...
def _install_tas(workspace: Path, task):
    """Install Goblin scaffolding into the workspace."""
    # CLAUDE.md with project-specific guidance
    claude_md = workspace / "CLAUDE.md"
    claude_md.write_text(GOBLIN_CLAUDE_MD.format(project_name=task.template))
...
def main():
    parser = argparse.ArgumentParser(description="Goblin benchmark runner")
