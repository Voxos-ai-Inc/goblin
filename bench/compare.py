...
Usage:
    python compare.py <label>                    # compare vanilla vs Goblin within a run
...
from config import GOBLIN, RESULTS_DIR, VANILLA
...
def cmd_compare_within(args):
    """Compare vanilla vs Goblin within a single labeled run."""
    results = load_results(args.label)

    vanilla = [r for r in results if r["condition"] == VANILLA]
    goblin_results = [r for r in results if r["condition"] == GOBLIN]

    if not vanilla or not goblin_results:
        print("Need both vanilla and Goblin results for comparison.")
        sys.exit(1)

    agg_v = aggregate(vanilla)
    agg_h = aggregate(goblin_results)

    # Paired statistics: match by task_id + trial
    pairs = _build_pairs(vanilla, goblin_results)
    stats = {}
    if pairs:
    ...
    print(f"\n=== {args.label}: Vanilla vs Goblin ===\n")
    print_comparison("vanilla", agg_v, "tas", agg_h, stats)

    if args.by_category:
        _print_category_breakdown(vanilla, goblin_results)
...
def _build_pairs(vanilla: list, goblin_results: list) -> list:
    """Match vanilla/Goblin results by task_id + trial for paired tests."""
    v_index = {}
    for r in vanilla:
...
def _print_category_breakdown(vanilla: list, goblin_results: list):
    """Print per-category comparison."""
    categories = sorted(set(r["category"] for r in vanilla + goblin_results))

    for cat in categories:
        v_cat = [r for r in vanilla if r["category"] == cat]
        h_cat = [r for r in goblin_results if r["category"] == cat]
